Function srv.2008r2.dc.install {

<#
Version: 0.8
Only works on Version R2
Original-Author : Tom Arbuthnot tomtalks.uk
#>
...
Param 	(
		...
		[Parameter(Mandatory=$false,
                   HelpMessage="Domain functional level. Based on the levels that exist in the forest when a new domain is created in an existing forest.")]
        [string[]]$Domainlevel = '4',
		
		[Parameter(Mandatory=$false,
                   HelpMessage="Must not use when installing new DC in existing forest. ForestLevel entry replaces SetForestVersion entry from Server2003.")]
